...
const Plans = ({ currentPlan, action, resetComponent, id }) => {
    ...
    if (form[name] === "") {
      ...
      try {
        ...
        const gas = await contract.methods
          .setDropValue(id + 1, form[name])
          .estimateGas({ from: owner });
        const result = await contract.methods
          .setDropValue(id + 1, form[name])
          .send({ from: owner, gasPrice, gas });
        if (result.status === true) {
          const plan = await contract.methods.plans(id + 1).call();
  ...
  const setRiskValue = async (name) => {
    if (form[name] === "") {
      return toast.error("Field can't be empty");
    } else if (form[name] > 100) {
      return toast.error("Cant be greater then 100");
    } else {
      if (form[name] > 100) {
  ...
  const setRewardValue = async (name) => {
    if (form[name] === "") {
      ...
      return toast.error("Cant be zero");
    } else {
      if (form[name] <= 0) {
  ...
  const addPlan = async (name) => {
    if (
      ...
      if (form["DropPercentage"] <= 0 || form["DropPercentage"] > 100) {
        return toast.error("Drop must be in between 1 and 100");
      }
      if (form["RiskPercentage"] > 100) {
        return toast.error("Risk cant be grearter then 100");
      }
      if (form["RewardPercentage"] <= 0) {
        return toast.error("Reward should be gretaer then zero");
      }
      ...
      try {
        ...
        if (result.status === true) {
          ...
          result = await contract.methods.plans(result - 1).call();
