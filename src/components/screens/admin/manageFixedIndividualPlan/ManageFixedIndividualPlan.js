...
const ManageFixedIndividualPlan = () => {
  ...
  const changeCoinStatus = async (e, index, action) => {
    try {
      ...
      if (result.status === true) {
        ...
        resetComponent(index, plan, "changeStatus");
        dispatch(setLoader(false));
      } else {
        dispatch(setLoader(false));
      }
  ...
  return !updateComponent ? (
    <Container fluid className="contentWrapper_main">
      <Container className="table_style" fluid>
        <NavbarTop />
        <HeaderAdmin title="Manage Fixed Individual Plan"></HeaderAdmin>
        <Table responsive>
          <thead>
            <tr>
            ...
            {plans.map((elem, index) => (
              <tr key={index}>
                ...
                <td>
                  <Form.Check
                    type="switch"
                    onChange={(e) => changeCoinStatus(e, index, !elem.isActive)}
                    label=""
...
export default ManageFixedIndividualPlan;
